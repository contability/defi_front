...
export default function FixedRapt() {
  ...
  const queryClaimableAmount = async (isClaim) => {
    ...
    query_with_arg({
      ...
      if (!res) return;
      let claimTmp = getethrep("" + res || 0);
      let claimAmount = (+claimTmp).toFixed(4);
      setClaimAbleAmount(claimAmount);
      if(isClaim){
        onClickClaim(claimAmount);
      }
      return res;
    });
  };
  ...
  const approve = async () => {
    ...
    // contractInteraction.approve(addresses.DEPOSIT00, addresses.STAKE_AND_REWARD, 100);
  };
  ...
  function allowance() {
    ...
    query_with_arg({
      ...
      aargs: [myAddress, addresses.STAKE_AND_REWARD],
    }).then((res) => {
  ...
  const getDepositCount = async () => {
    ...
    query_with_arg({
      contractaddress: addresses.STAKE_AND_REWARD,
      abikind: "STAKE_AND_REWARD",
      methodname: "_cumul_deposit_count",
      aargs: [],
    }).then((res) => {
      console.log("depositCount res : ", res);
      setDepositCount(res);
    });
  };

  const getWithdrawCount = async () => {
    query_with_arg({
      contractaddress: addresses.STAKE_AND_REWARD,
      abikind: "STAKE_AND_REWARD",
      methodname: "_cumul_withdraw_count",
      aargs: [],
    }).then((res) => {
      console.log("withdrawCount res : ", res);
      setWithdrawCount(res);
    });
  };

  const queryClaimedReward = async () => {
    query_with_arg({
      contractaddress: addresses.STAKE_AND_REWARD,
      abikind: "STAKE_AND_REWARD",
      ...
      console.log("query_claimed_reward res : ", res);
      let temp = getethrep("" + res || 0);
      setClaimedReward((+temp).toFixed(4));
    });
  };
  ...
  const getClaimCount = async () => {
    ...
    query_with_arg({
      contractaddress: addresses.STAKE_AND_REWARD,
      abikind: "STAKE_AND_REWARD",
      methodname: "_cumul_claim_count",
      aargs: [],
    }).then((res) => {
      console.log("claimCount res : ", res);
      setClaimCount(res);
    });
  };
  ...
  const getRewardAmount = async () => {
    query_with_arg({
      contractaddress: addresses.STAKE_AND_REWARD,
      abikind: "STAKE_AND_REWARD",
      methodname: "_unit_reward_amount",
      aargs: [],
    }).then((res) => {
      console.log("rewardAmount res : ", res);
      let temp = getethrep("" + res || 0);
      setRewardAmount((+temp).toFixed(4));
    });
  };

  function queryDepositAmount() {
    let myAddress = getmyaddress(wallet);
    if (!myAddress) myAddress = getLocalStorage("address")?.value;
    ...
    query_with_arg({
      contractaddress: addresses.STAKE_AND_REWARD,
      abikind: "STAKE_AND_REWARD",
      methodname: "_balancesums",
      aargs: [myAddress],
    }).then((res) => {
      console.log("withdrawable is : ", res);
      let temp = getethrep("" + res || 0);
      setDepositAmount(floorAmount(temp));
    });
  }
  ...
  const ratesDate = () => {
    ...
    .then((res) => {
      ...
      if(res?.data?.list){
        ...
        list.map((v, i) => {
          switch (v.key_){
            case "FIXED_RATES_END":
              setRatesEnd(v.value_);
              break;
            case "FIXED_RATES_START":
  ...
  useEffect(() => {
    ...
    allowance();
    queryClaimableAmount(false);
    
    // contractBalance(addresses.DEPOSIT00, setDepositBal);
    // contractBalance(addresses.REWARD00, setRewardBal);
      ...
      <main>
        ...
        <div className="mainArea">
          ...
          <article className="content">
            <strong className="contentTitle">Fixed Plan</strong>
            <span>
              ...
              <aside className="leftBox">
                <p className="flexSection">
                  ...
                  <section className="urStake contentSct">
                    ...
                    <div>
                      <p className="panelHeader">Deposit Count</p>
                      <p>{depositCount}</p>
                    </div>
                    <div className="flxHeader">
                      <p>
                        <p className="panelHeader">Withdraw Count</p>
                        <p>{withdrawCount}</p>
                      </p>
                      <p className="panelFlx">{/* <button>Burn</button> */}</p>
                    </div>
                  </section>
                </p>
                <section className="stakingAssets contentSct">
                  <strong className="sectionHeader">
                    My Contract Interaction
                  </strong>
                  <div>
                  ...
                  <div>
                    ...
                    {/* <p>{(+(getethrep(""+depositAmount))).toFixed(4) || "0.0000"}</p> */}
                    <p>{depositAmount}</p>
                  </div>
                </section>
              </aside>
                ...
                <section className="urRewards contentSct">
                  ...
                  <div>
                    ...
                    <p className="panelFlx">
                      <p>{claimAbleAmount || "0.0000"}</p>
                    </p>
                  </div>
                  <div>
                    <p className="panelHeader">Claimed Reward</p>
                    <p className="panelFlx">
                      <p>{claimedReward}</p>
                    </p>
                  </div>
                  <div className="boxArea">
                    {isClaiming ? (
                      ...
                      <button className="claimBtn" onClick={() => queryClaimableAmount(true)}>
                        CLAIM
                      </button>
                    )}
                  </div>
                  <div className="boxArea">
                    <p className="detailBox">
                      <span className="panelFlx detailContent pt_13">
                        <span className="panelHeader">Claim Count</span>
                        <p>{claimCount}</p>
                      </span>
